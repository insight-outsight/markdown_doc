...
在编译阶段和运行阶段都可以使用-classpath来指定本地软件包(也可以是文件夹)作为类寻找路径，以便找到代码中引用的类文件。
...
Java虚拟机（以下简称为JVM）是一个虚拟的计算机设备，它屏蔽了不同操作系统的差异，使同样的代码（字节码）能在不同类型的操作系统上都能运行，这只需要在不同的操作系统上安装相应的JVM即可实现。

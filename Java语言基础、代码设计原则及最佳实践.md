...
public class TextHelloImpl implements IHello{
    ...
    public void sayHello() {
        System.out.println("Here say hello.");  
    }
...
Java中类(属性、方法)访问权限作用域修饰符有四种，分别是public、protected、private和defaut(默认的访问权限是包)。除了默认的包作用域外，分别使用public、protected、private关键字表示。不同的修饰符拥有的访问权限作用域如下表所示。
...
Java中类的多态是本质通过面向接口编程实现的，下面用一个实际例子来阐述多态的含义。
